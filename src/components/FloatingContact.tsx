...
export default function FloatingContact({ onOpenFullContact }: FloatingContactProps) {
  ...
  const toggleOpen = () => {
    setIsOpen((prev) => {
      ...
      if (nextState) {
        setShowBubble(false); // 展开卡片时自动隐藏提示气泡
      }
      return nextState;
    });
  };
  ...
  return (
    ...
    >
      {/* 1. 主动关怀引导气泡 (Proactive Prompt Bubble) */}
      {showBubble && !isOpen && (
        <div className="mb-3 max-w-[270px] sm:max-w-xs animate-fade-in transition-all duration-300 transform origin-bottom-right">
      ...
      {/* 2. 快捷客服微名片展开卡片 (Quick Popover Card) */}
      {isOpen && (
        <div className="mb-3 w-[310px] sm:w-[340px] rounded-2xl bg-surface border border-theme-subtle shadow-2xl overflow-hidden animate-fade-in transition-all origin-bottom-right flex flex-col">
          {/* Header */}
          <div className="p-4 border-b border-theme-subtle bg-surface-elevated flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="relative w-8 h-8 rounded-lg bg-surface border border-theme-subtle flex items-center justify-center shadow-xs">
      ...
      {/* 3. 常态悬浮挂件按钮 (Floating Trigger Button) */}
      <button
        type="button"
        onClick={toggleOpen}
